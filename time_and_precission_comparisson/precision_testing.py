...
with open("results_1.csv", "w", newline="") as csvfile:
    ...
    csv_writer.writerow(["Common latitude", "Authalic (direct mpmath)", "Authalic (OLD)", "Authalic (NEW)", "Authalic (Clenshaw)",
                         "Diff (auth OLD - auth NEW) E-12", "Diff (auth OLD - auth direct mpmath) E-12",
                         "Diff (auth NEW - auth direct mpmath) E-12", "Diff (auth Clenshaw - auth direct mpmath) E-12",  "Authalic inverse (OLD)",
    ...
    for phi in common_latitudes:
        ...
        auth_latitude_new_clenshaw = auth_lat_new_clenshaw(phi, eccentricity)
        ...
        diff_newclenshaw_mpmath = str((mp.mpf(str(auth_latitude_new_clenshaw)) - mp.mpf(str(auth_latitude_mpmath))) * 10 ** 12)
        ...
        write_list = [phi, auth_latitude_mpmath, auth_latitude_old, auth_latitude_new, auth_latitude_new_clenshaw, diff_old_new, diff_old_mpmath,
                      diff_new_mpmath, diff_newclenshaw_mpmath, auth_latitude_inverse_old, auth_latitude_inverse_new, diff_inverse_old_new,
