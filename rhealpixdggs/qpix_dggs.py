...
from rhealpixdggs.qpix_cell import Cell, CELLS0
from rhealpixdggs.ellipsoids import (
    WGS84_ELLIPSOID,
...
class QPixDGGS(object):
    ...
    def __init__(
        ...
        ul_vertex = {  # Location for radius = 1
            CELLS0[0]: (-2 * math.sqrt(2 * math.pi / 3) + self.north_square * math.sqrt(2 * math.pi / 3), 3 * math.sqrt(2 * math.pi / 3) / 2),
            CELLS0[1]: (-2 * math.sqrt(2 * math.pi / 3), math.sqrt(2 * math.pi / 3) / 2),
            CELLS0[2]: (-math.sqrt(2 * math.pi / 3), math.sqrt(2 * math.pi / 3) / 2),
            CELLS0[3]: (0.0, math.sqrt(2 * math.pi / 3) / 2),
            CELLS0[4]: (math.sqrt(2 * math.pi / 3), math.sqrt(2 * math.pi / 3) / 2),
            CELLS0[5]: (-2 * math.sqrt(2 * math.pi / 3) + self.south_square * math.sqrt(2 * math.pi / 3), -math.sqrt(2 * math.pi / 3) / 2),
        }
    ...
    def rosca_plonka(self, u, v, inverse=False):
        """
        Return the Rosca-Plonka projection of the point `(u, v)` (or its inverse if
        `inverse` = True) appropriate to this DGGS.
        """
        f = pw.Projection(
            ellipsoid=self.ellipsoid,
            proj="rosca_plonka",
    ...
    def xyz(self, u, v, lonlat=False):
        ...
        if lonlat:
            ...
            lam, phi = self.rosca_plonka(u, v, inverse=True)
    ...
    def cell_width(self, resolution, plane=True):
        ...
        """
        if plane:
            return self.ellipsoid.R_A * (math.sqrt(2 * math.pi / 3)) * self.N_side ** (-resolution)
    ...
    def cell_from_point(self, resolution, p, plane=True):
        ...
        square_a = math.sqrt(2 * math.pi / 3)
        if (
            y > R * (1 / 2 * square_a)
            and y < R * (3 / 2 * square_a)
            and x > R * (-2 * square_a + ns * square_a)
            and x < R * (-square_a + ns * square_a)
        ):
            s0 = CELLS0[0]
        elif (
            y < R * (1 / 2 * square_a)
            and y > R * (3 / 2 * square_a)
            and x > R * (-2 * square_a + ss * square_a)
            and x < R * (-square_a + ss * square_a)
        ):
            s0 = CELLS0[5]
        elif y >= -R * square_a / 2 and y <= R * square_a / 2 and x >= -R * 2 * square_a and x < -R * square_a:
            s0 = CELLS0[1]
        elif y >= -R * square_a / 2 and y <= R * square_a / 2 and x >= -R * square_a and x < 0.0:
            s0 = CELLS0[2]
        elif y >= -R * square_a / 2 and y <= R * square_a / 2 and x >= 0.0 and x < R * square_a:
            s0 = CELLS0[3]
        elif y >= -R * square_a / 2 and y <= R * square_a / 2 and x >= R * square_a and x < R * 2 * square_a:
            s0 = CELLS0[4]
        else:
    ...
    def cell_from_region(self, ul, dr, plane=True):
        ...
        if not plane:
            ...
            vertices = [self.rosca_plonka(*p) for p in vertices]
    ...
    def cell_latitudes(self, resolution, phi_min, phi_max, nucleus=True, plane=True):
        # TODO adapt (parallels are not mapped to straight lines in Rosca-Plonka)
        """
        ...
        if plane:
            ...
            y_min = self.rosca_plonka(0, phi_min)[1]
            y_max = self.rosca_plonka(0, phi_max)[1]
        w = self.cell_width(resolution)
        ...
        if not plane:
            x = -R * 3 / 2 * square_a + R * square_a * self.north_square
            result = [self.healpix(x, y, inverse=True)[1] for y in result]
        return result

    def cells_from_meridian(self, resolution, lam, phi_min, phi_max):
        # TODO adapt (meridians are not mapped to straight lines in Rosca-Plonka)
        """
...
WGS84_003_RADIANS = QPixDGGS(
    ellipsoid=WGS84_ELLIPSOID_RADIANS, north_square=0, south_square=0, N_side=3
)
UNIT_003 = QPixDGGS(ellipsoid=UNIT_SPHERE, north_square=0, south_square=0, N_side=3)
UNIT_003_RADIANS = QPixDGGS(
