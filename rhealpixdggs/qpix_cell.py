...
class Cell(object):
    ...
    def ul_vertex(self, plane=True):
        ...
        if not plane:
            x, y = self.rdggs.rosca_plonka(x, y, inverse=True)
        return x, y
    ...
    #         # the cell lies in.
    #         rdggs = self.rdggs
    #         triangle, region = rdggs.triangle(*self.nucleus(plane=True))  # TODO is it possible that nucleus lays on the edge
    ...
    #         ev = [self.rdggs.rosca_plonka(*vv, inverse=True) for vv in v]
    ...
    #             result = v[(i + 1) % 4]
    #     if not plane:
    #         result = self.rdggs.rosca_plonka(*result, inverse=True)
    #     return result

    def nucleus(self, plane=True):
        """
        Return the nucleus of this planar or ellipsoidal cell
    ...
    def boundary(self, n=2, plane=True, interior=False):
        ...
        if n < 2:
            n = 2  # TODO is this wrong in the original codebase (originally: n == 2)
        if interior:
        ...
        result.pop()
        # TODO adapt, now all planar boundary points are just projected back to ellipsoid, without reordering
        # if not plane:
        #     # Reorder result so that it starts with the northwest vertex.
        #     # Clockwise ordering is preserved when mapping from plane to
        #     # ellipsoid.
    ...
    def interior(self, n=2, plane=True, flatten=False):
        ...
        def g(x, y):
            if plane:
                ...
                return self.rdggs.rosca_plonka(x, y, inverse=True)
    ...
    def random_point(self, plane=True):
        ...
        """
        vertices = self.vertices(plane=True)
        ...
        random_point = uniform(u_min, u_max), uniform(v_min, v_max)
        if plane:
            return random_point
        else:
            return self.rdggs.rosca_plonka(*random_point, inverse=True)

            # TODO currently, random point from plane is just mapped to the ellipsoid
            # TODO it should be sampled from the ellipsoidal cell
            # if self.ellipsoidal_shape() == "cap":
