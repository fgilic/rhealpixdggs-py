...
def get_s2_cells(s2_tci):
    ...
    cells = rdggs.cells_from_region(9, (ul_wgs[1], ul_wgs[0]), (lr_wgs[1], lr_wgs[0]), plane=False)
    ...
    with fiona.open(
        ...
        for cell_polygon in cells_polygons:
            cell = {
                ...
                'properties': dict([
                    ('color', cell_polygon[1])
                ])
            }
            cells_geojson.write(cell)


s2_tci, red_band, green_band, blue_band = read_RGB_raster("../T33TXJ_20231004T095031_TCI.jp2")
...
for cell_group in cells:
    for cell in cell_group:
        ...
        cells_polygons.append([cell_polygon, f'{red},{green},{blue},255'])
